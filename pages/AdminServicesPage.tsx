...
const AdminServicesPage: React.FC = () => {
    ...
    const handleDelete = (slug: string, title: string) => {
        if (window.confirm(`Are you sure you want to delete the service "${title}"? This action cannot be undone.`)) {
            deleteService(slug);
        }
    }
                        ...
                        <table className="w-full text-sm text-left text-dark-gray">
                            ...
                            <tbody>
                                {services.length > 0 ? services.map(service => (
                                    <tr key={service.slug} className="bg-white border-b hover:bg-primary/5">
                                        ...
                                        <td className="px-6 py-4 text-right space-x-3">
                                            <Link to={`/admin/services/edit/${service.slug}`} className="font-medium text-primary hover:underline">Edit</Link>
                                            <button onClick={() => handleDelete(service.slug, service.title)} className="font-medium text-red-600 hover:underline">Delete</button>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr>
                                        <td colSpan={3} className="text-center py-10 text-dark-gray/70">
